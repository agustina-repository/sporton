...
const LoginPage = () => {
  ...
  const handleLogin = async () => {
    ...
    try{
      ...
      setErrorMessage(err.message || "Something went wrong, please try again latter")
  ...
  return (
    <main className="bg-[#F7F9FA] w-full min-h-screen flex justify-center items-center">
      <div className="max-w-136 w-full bg-white rounded-xl border-t-4 border-primary py-12 px-[72px]">
        ...
        {errorMessage && (
          <div className="px-3 py-1 bg-primary-light border border border-primary round md text-primary test-sm text-center w-full">
          {errorMessage}
        </div>
        )}
